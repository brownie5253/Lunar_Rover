...
class VREP_RoverRobot(object):
	...
	def GetDetectedObjects(self):
		...
		if self.cameraPose != None:
			...
			retCode,objectsDetected,_,_,_ = vrep.simxCallScriptFunction(self.clientID, 'Robot', vrep.sim_scripttype_childscript, 'getObjectsInView', [], [], [], bytearray(),
	...
	def DropSample(self):
		if self.sampleConnectedToRobot:
	 			vrep.simxCallScriptFunction(self.clientID, 'Robot', vrep.sim_scripttype_childscript, 'JoinRobotAndSample', [0], [0.0], [], bytearray(),
                                            vrep.simx_opmode_blocking)
	 			self.sampleConnectedToRobot = False
	...
	def CollectSample(self):
		...
		for _, samplePosition in enumerate(self.samplePositions):
			...
			if sampleDist != None and sampleDist < self.robotParameters.maxCollectDistance and self.sampleConnectedToRobot == False:
				...
				vrep.simxCallScriptFunction(self.clientID, 'Robot', vrep.sim_scripttype_childscript, 'JoinRobotAndSample', [1], [self.robotParameters.maxCollectDistance], [], bytearray(),
                                            vrep.simx_opmode_blocking)
			...
			if samplePosition != None:
				...
				if self.robotParameters.autoCollectSample == True and sampleDist != None and sampleDist < self.robotParameters.maxCollectDistance and self.sampleConnectedToRobot == False:
					...
					if np.random.rand() > self.robotParameters.collectorQuality:
						...
						vrep.simxCallScriptFunction(self.clientID, 'Robot', vrep.sim_scripttype_childscript, 'JoinRobotAndSample', [0], [self.robotParameters.maxCollectDistance], [], bytearray(),
                                                    vrep.simx_opmode_blocking)
						self.sampleConnectedToRobot = False
